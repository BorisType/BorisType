...
const res1 = callWith(function (a: number, b: number) {
    return a + b;
}, 3, 4);
...
const res2 = callWith(function sum(a: number, b: number) {
    return a * b;
}, 3, 4);
...
const res6 = runChain([
    function (v: number) { return v + 1; },
    function (v: number) { return v * 2; },
    (v: number) => v + 10,
], 5);
...
const res7 = executeHandler({
    run: function (x: number) { return x + 100; }
}, 5);
