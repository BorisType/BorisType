...
1. **Нестрогая проверка равенства**
   ```javascript
   if (x == null) { /* true для null и undefined */ }
   ```
...
4. **Проверка на ложность**
   ```javascript
   if (!x) { /* falsy: null, undefined, 0, "", false, NaN */ }
   ```
   Pro: Кратчайший  
...
var __tmp;
var result = (__tmp = obj) == null || __tmp == undefined
  ? undefined
  : bt.getProperty(__tmp, "prop");
```
...
```javascript
if (x == null) { /* покрывает null и undefined */ }
```
...
```javascript
var result = bt.isNullish(obj)
  ? undefined
  : bt.getProperty(obj, "prop");
```
