...
export function flatMap<T, U>(array: Array<T>, callback: (value: T, index: number, array: Array<T>) => U): Array<U> {
  ...
  for (let i = 0; i < len; i++) {
    const value = array[i];
    const mapped = bt.callFunction(callback, [value, i, array]);
    if (IsArray(mapped)) {
      for (const item of mapped as Array<U>) {
        result.push(item);
      }
    } else {
...
// Нужен чисто для будущей работы с протоипами
...
export function reduce<T, U>(
  ...
  for (let i = startIndex; i < len; i++) {
    const value = array[i];
    accumulator = bt.callFunction(callback, [accumulator, value, i, array]) as U;
  }
