...
export function parseUserPackageInfo(
  ...
  if (fs.existsSync(packageJsonPath)) {
    try {
      ...
      if (packageType === 'library') {
        logger.error(`❌ Package ${displayName}: libraries (ws:package: "library") cannot be linked directly. They are included via node_modules of executable packages.`);
        process.exit(1);
      } else if (packageJson['ws:package'] && !packageType) {
        logger.error(`❌ Package ${displayName}: invalid ws:package type "${packageJson['ws:package']}". Valid types: ${formatValidPackageTypes()}`);
        process.exit(1);
      }
    } catch (error) {
