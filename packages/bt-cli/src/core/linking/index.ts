...
export function resolvePackagesToLink(projectPath: string): BtConfigLinkingPackage[] {
  ...
  if (!packageType || !isExecutablePackageType(packageType)) {
    logger.error('❌ No btconfig.json found and current project is not an executable BorisType package (ws:package must be "standalone", "component", "system"). Nothing to link.');
    process.exit(1);
  }
